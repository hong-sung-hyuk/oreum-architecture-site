...
if "selected_project_gallery" not in st.session_state:
    st.session_state.selected_project_gallery = None
if "selected_project" not in st.session_state:
    st.session_state.selected_project = None
if "admin_logged_in" not in st.session_state:
    st.session_state.admin_logged_in = False
if "ai_applications" not in st.session_state:
    st.session_state.ai_applications = []  # ✅ 신청 정보 리스트
...
if is_admin_url and not st.session_state.admin_logged_in:
    ...
    st.stop()  # 로그인 성공 전에는 전체 페이지 렌더링 중단
...
st.markdown("""
    <style>
        ...
        /* .stDownloadButton, .stFormSubmitButton 등도 포함 */
...
for i, tab in enumerate(tabs):
    with cols[i]:
        if st.button(tab, use_container_width=True, key=f"tab_button_{i}_{tab}"):
            ...
            if tab == "Works" and previous_tab != "Works":
                st.session_state.selected_project = None
...
if st.session_state.current_tab == "News":
    ...
    for category, tab in zip(NEWS_CATEGORIES, tabs):
        with tab:
            ...
            if (
                st.session_state.selected_post is None
                or st.session_state.selected_post.get("category") != category
            ):
                # 리스트 출력
                if not filtered_posts:
                    ...
                    for idx, post in enumerate(reversed(filtered_posts)):
                        with cols[idx % 6]:
                            ...
                            st.markdown(
                                f"""<div style='text-align:center; font-size:14px; margin-top:8px;'>
                                    <b>{post['title'][:20]}</b><br>
                                    <span style='font-size:12px; color:gray;'>{post['date']}</span>
                                </div>""",
                                unsafe_allow_html=True
                            )
